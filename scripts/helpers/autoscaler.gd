...
##
## Will automatically set the scale of the game's UI elements when the window is resized, 
## or set the scale to the player's choice if autoscaling is turned off.

## The factor by which to automatically scale the UI.[br]
## The current setting means a window 1080 pixels tall or wide will have a UI scale of 1.0
const SCALE_FACTOR := 1080.0

## A static reference to the AutoScaler singleton.[br]
## Static
...
func resize() -> void:
	if ProjectSettings.get_setting("gameplay/ui/auto_scale"):
		var vp := get_viewport()
		var smallest_side := mini(vp.size.x, vp.size.y)
		get_window().content_scale_factor = smallest_side / SCALE_FACTOR
	else:
		get_window().content_scale_factor = ProjectSettings.get_setting("gameplay/ui/scale")
	if is_instance_valid(Hotbar.instance):
		Hotbar.instance.defer_redraw()
